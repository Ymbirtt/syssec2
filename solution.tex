...
will be routed to the correct devices. An alternative example of software that performs these attacks is EtterCap, which
we used in addition to netwox. \ref{fig:etterarp}
...
network. Despite the configuration of the virtual network making this attack harder, we can see the ICMP redirect
packets in a ping response between client and server.

% In the example shown, the `server' has advertised it is the best
% route to all hosts. When we ping it from the client, we can see these ICMP packets being returned at every step.\ref{fig:pingdirect}
...
%PUT PICTURES OF FTP NOT GIVING A DAMN ABOUT MY PUNY SQ PACKETS HERE - EVEN WHEN TRANSMITTING A 1GiB FILE
...
%POOT CODE HERE
\begin{lstlisting}[caption=The Sunshine And Rainbows Worm]
